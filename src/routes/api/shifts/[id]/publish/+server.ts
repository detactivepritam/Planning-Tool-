...
export const POST: RequestHandler = async (event) => {
  ...
  try {
    const updated = await setShiftPublishState(auth.user.organizationId, shiftId, true);
    if (!updated) {
    ...
    return json({ error: { code: 'SERVER_ERROR', message: 'Failed to publish shift' } }, { status: 500 });
  }
};
